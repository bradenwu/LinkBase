...
int main(int argc, char** argv) {
    ...
    const uint32_t kDeadLinkIntervalThreshold  = 86400 * 7;
	...
	//cerr << "base64String.at() (in int) = " << (int)base64String.at(base64String.size()-1) << endl;
        ...
        //pbItem.set_deadlinktime(1398425322);
        //cerr << "[DEBUG]currentTime="<< current_time << ",DebugString" << pbItem.DebugString() << endl;
        if (pbItem.has_deadlinktime()) {
            uint32_t deadlinktime = pbItem.deadlinktime();
            if (current_time > deadlinktime && (current_time - deadlinktime > kDeadLinkIntervalThreshold)) {
                cout << "DEL\t" << fields.at(0) << "\t{\"delReason\":\"expiredDeadLink\"}" << endl; 
                continue;
            }
        }

//        pbItem.set_deadpagetime(1378425322);
        if (pbItem.has_deadpagetime()) {
            uint32_t deadpagetime = pbItem.deadpagetime();
            if (current_time > deadpagetime && (current_time - deadpagetime > kDeadLinkIntervalThreshold)) {
                cout << "DEL\t" << fields.at(0) << "\t{\"delReason\":\"expiredDeadPage\"}" << endl; 
                continue;
            }
